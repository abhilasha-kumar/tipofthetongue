...
TOTSemantic_z_trimmed = subset(TOTSemantic_z, 
...
## merge aggregate info with long data
TOTSemantic_final_z = merge(TOTSemantic_z_trimmed, 
                             RT_agg, by = "Subject", all.x = T)

## person and grand-mean centered scores using original and aggregate
library(dplyr)
TOTSemantic_final_z = TOTSemantic_final_z %>% mutate(zPrimeRT_trim = 
                                             (PrimeDefRT - 
...
sub_pic = group_by(TOTSemantic_final_z, Subject) %>%
