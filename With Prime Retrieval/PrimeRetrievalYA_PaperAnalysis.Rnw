...
agg_prime_ya= Rmisc::summarySE(PrimeRetrieval, 
                      measurevar = "Accuracy",
                      groupvars = c("PrimeFirstResp_ACC", "PrimeCondition"))

agg_prime_ya$PrimeFirstResp_ACC = as.factor(agg_prime_ya$PrimeFirstResp_ACC)
library(ggplot2)
library(ggthemes)

agg_prime_ya %>% mutate(PrimeType = factor(PrimeCondition, 
...
<<>>=
# setwd("~/WU-Projects/TOT/MTURK Ratings/Julie Ratings")
...
main_item = merge(main, itemratings, by = "Stimuli2")
