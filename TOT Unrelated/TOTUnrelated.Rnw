...
TOTUnrelated_z_trimmed = subset(TOTUnrelated_z, 
...
## merge aggregate info with long data
TOTUnrelated_final_z = merge(TOTUnrelated_z_trimmed, 
                             RT_agg, by = "Subject", all.x = T)

## person and grand-mean centered scores using original and aggregate
library(dplyr)
TOTUnrelated_final_z = TOTUnrelated_final_z %>% mutate(zPrimeRT_trim = 
                                             (PrimeDefRT - 
...
sub_pic = group_by(TOTUnrelated_final_z, Subject) %>%
